...
def make_flow_model(
  ...
  def bijector_fn(params: Array):
    return distrax.RationalQuadraticSpline(
      params,
      range_min=0.,
      range_max=2 * np.pi if periodized else 1.,
...
def RQSFlow(
  ...
  def model() -> Array:
    ...
    def forward_jac(x):
      return jax.vmap(jax.jacfwd(flow.bijector.forward))(x)

    def inverse_jac(y):
      return jax.vmap(jax.jacfwd(flow.bijector.inverse))(y)

    def gauge_potential(x):
      return jax.jacfwd(lambda x_: flow.bijector.forward_and_log_det(x_)[1])(x)
