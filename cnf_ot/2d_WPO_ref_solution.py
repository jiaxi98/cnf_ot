...
if not os.path.isfile('data/fcn4a1_interp.pkl'):
  ...
  def rho0_np(x): # initial density N(0,1) in numpy
    return np.exp(-np.sum(x**2,axis=-1,keepdims=True)/2)/((2*np.pi)**(d/2))
  ...
  y = torch.tensor([[0.0, 0.0], [-0.5,-0.5], [0.5,0.5]], dtype=dtype, device=device) # Ny x 2
  yt_y1 = torch.linspace(-int_range,int_range,int_num2, dtype=dtype, device=device) # int_num2 x 1
  yt_y2 = torch.linspace(-int_range,int_range,int_num2, dtype=dtype, device=device) # int_num2 x 1
  yt_y1, yt_y2 = torch.meshgrid(yt_y1, yt_y2) # int_num2 x int_num2
  yt_y = torch.stack((yt_y1.flatten(), yt_y2.flatten()), dim=1) # int_num2^2 x 2
  # yt_y2 = yt_y**2

  yt_y_unsq = yt_y.unsqueeze(0) # 1 x int_num2^2 x 2
  y_unsq = y.unsqueeze(1) # Ny x 1 x 2
  yt = yt_y_unsq + y_unsq # Ny x int_num2^2 x 2
  temp = g(yt) + torch.sum(yt_y_unsq**2,dim=-1,keepdim=True)/(2*T) # Ny x int_num2^2 x 1
  ...
  # save the result
  np.savez('./results/WPONG/hy2d.npz', a=a, dy=dy, y_range=y_range, beta=beta, coef=coef, hy=hy)
  print('h(y) saved')
  ...
  for i in range(int_numx2**2):
    ...
    temp = np.exp(-(gx[i:(i+1),:] + x_i_y2 / (2*T)) / (2*beta)) * rho0y / hy# int_numy2^2 x 1
    rhoT[i,0] = np.sum(temp) * dy2
    temp2 = - temp * (g_primex[i:(i+1),:] + x_i_y/T) / (2*beta) # int_numy2^2 x 2
  ...
  scoreT_1 = scoreT[:,0].reshape((int_numx2,int_numx2)) # int_numx2 x int_numx2
  scoreT_2 = scoreT[:,1].reshape((int_numx2,int_numx2)) # int_numx2 x int_numx2
  ...
  with open('data/fcn4_interp.pkl', 'wb') as f:
    pickle.dump({'rhoT_interp':rhoT_interp, 
                'scoreT_1_interp':scoreT_1_interp, 'scoreT_2_interp':scoreT_2_interp, 
                'w0_1_interp':w0_1_interp, 'w0_2_interp':w0_2_interp, 
                'wT_1_interp':wT_1_interp, 'wT_2_interp':wT_2_interp}, f)
